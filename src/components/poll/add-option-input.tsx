...
interface AddOptionInputProps {
  ...
  isAdding: boolean;
  hasVoted: boolean;
  onAdd: (sectionId: string, name: string, emoji?: string, imageUrl?: string) => void;
...
export function AddOptionInput({ sectionId, sectionName, color, isAdding, onAdd, hasAddedCustomOption }: AddOptionInputProps) {
  ...
  function submit() {
    ...
    onAdd(sectionId, trimmed, emoji, imageUrl.trim() || undefined);
    setName("");
    setEmoji("✨");
    setImageUrl("");
    setOpen(false);
  ...
  return (
    <div className={cn("rounded-xl border border-dashed bg-muted/30 p-3 space-y-2.5")}>
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-muted-foreground">{t.addTo(sectionName)}</span>
        <button type="button" onClick={() => { setOpen(false); setName(""); setEmoji("✨"); setImageUrl(""); }}
      ...
      <div className="flex flex-col gap-2">
        <Input autoFocus value={name} onChange={(e) => setName(e.target.value)}
          ...
          placeholder={t.addPlaceholder} maxLength={60} disabled={isAdding} className="h-9 text-xs sm:text-sm" />

        <div className="flex items-center gap-2">
          <Input value={imageUrl} onChange={(e) => setImageUrl(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); submit(); } }}
            placeholder="Image URL (optional, e.g. https://...webp)" disabled={isAdding} className="h-9 text-xs sm:text-sm" />
