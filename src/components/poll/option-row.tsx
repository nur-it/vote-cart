...
export function OptionRow({ option, color, mode, isSelected, isUserPick, onToggle }: OptionRowProps) {
    ...
    return (
      <motion.label
        ...
        className={cn(
          ...
          isSelected ? cn(c.border, c.bg, "shadow-xs") : "border-border bg-card hover:bg-accent/50"
        )}
      >
        <Checkbox
          checked={isSelected}
        ...
        />
        <span className="text-lg leading-none sm:text-xl" aria-hidden>{option.emoji}</span>
        <span className="min-w-0 flex-1">
          ...
          <span className="block truncate text-[11px] text-muted-foreground sm:text-xs">{option.description}</span>
        </span>
        {isSelected && (
